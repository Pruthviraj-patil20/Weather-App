...
async function loadPlace(place, force = false) {
  ...
  try {
    ...
    console.warn("[skycast] data load error:", err);
    const cacheKey = `${place.lat.toFixed(2)},${place.lon.toFixed(2)}`;
    const cached = Storage.getCache(cacheKey);
    if (cached) {
      state.data = cached;
      renderCurrent();
      showToast("Weather Notice", `${err.message || "Network issue"} — showing recent saved data.`, "warning");
    } else if (state.data) {
      showToast("Weather Notice", err.message || "Failed to refresh.", "warning");
    } else {
      state.data = generateFallbackData(place.name, place.lat, place.lon);
      renderCurrent();
      showToast("Weather Notice", `${err.message || "API rate-limited"} — displaying demo data. Add your API key in Settings.`, "warning");
    }
  } finally {
    setLoading(false);
  }
